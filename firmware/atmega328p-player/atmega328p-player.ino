...
// Patched by the browser before STK500v1 programming.
struct __attribute__((packed)) CurveConfig {
  char marker[16];
  uint32_t cycleMs;
...
uint8_t brightnessAt(uint32_t elapsedMs) {
  ...
  // AVR int is 16-bit; widen before multiplying a full-scale transition.
  return (uint8_t)((int32_t)a + ((int32_t)b - a) * fraction / 256);
}
